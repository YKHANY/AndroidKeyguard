...
public class KeyguardViewMediator implements KeyguardViewCallback,
    ...
    public static int SECUREMODE = 0;  // 0 - Pleas Insert... 1 - Certificating... 2 - Try again... 3 - success
    ...
    private static final int VERIFYTOKEN=14;
    ...
    public KeyguardViewMediator(Context context, PhoneWindowManager callback,
        ...
        headSetFilter.addAction("org.secmem.intent.SELECT_CONN");
        headSetFilter.addAction(Intent.ACTION_HEADSET_PLUG);
        context.registerReceiver(mHeadSetReceiver, headSetFilter);
        ...
        IntentFilter modeChangeFilter = new IntentFilter();
        modeChangeFilter.addAction("org.secmem.intent.fake.MODECHANGE_SUCCESS");
	modeChangeFilter.addAction("org.secmem.intent.fake.MODECHANGE_FAILED");
	modeChangeFilter.addAction("org.secmem.intent.fake.SOCKET_FAILED");
	modeChangeFilter.addAction("org.secmem.intent.ALARM");
        context.registerReceiver(mModeChangeReceiver, modeChangeFilter);
        
        Log.d("Secmem", getDeviceUUID(context));
	//
    ...
    public static void setSecureMode (final int mode) {
    	KeyguardViewMediator.SECUREMODE = mode;
    }
    ...
    private BroadcastReceiver mHeadSetReceiver = new BroadcastReceiver(){
		...
		public void onReceive(Context context, Intent intent) {
			...
			Log.d(TAG,"Dialog Selected Conn!");
			String action = intent.getAction();
			
			if(action.equals("org.secmem.intent.SELECT_CONN")){
				// Dialog CONN Button Selected.
				// 또한 TextView를 처리중으로 바꿔주어야 함.
				setSecureMode(1);
				synchronized(KeyguardViewMediator.this){
					resetStateLocked();
				}
				pokeWakelock(20000);
				SecmemManager sm = (SecmemManager)context.getSystemService("secmem");
				String ip = intent.getStringExtra("ip");
				String port = intent.getStringExtra("port");
				String time = intent.getStringExtra("time");
				sm.trying2Unlock2(ip, port, time);
				Log.d(TAG,"go go secmem service!");
    ...
    public void verifySecureToken(){
    	if (DEBUG) Log.d(TAG, "verifyTocken");
        // ensure we stay awake until we are finished displaying the keyguard
        mShowKeyguardWakeLock.acquire();
        Message msg = mHandler.obtainMessage(VERIFYTOKEN);
        mHandler.sendMessage(msg);
    }
    
    private void handleVerifySecureToken(){
    	Log.i(TAG, "Service : trying2Unlock");
		DecryptToken dec = new DecryptToken(getDeviceUUID(mContext), Environment.getExternalStorageDirectory().getPath()+"/usb/certificate"
				, Environment.getExternalStorageDirectory().getPath()+"/certificate");
		try {
			if(dec.HybridDecrypt()) {
				setSecureMode(0);
				
				EncryptToken enc = new EncryptToken(getDeviceUUID(mContext), Environment.getExternalStorageDirectory().getPath()+"/usb/certificate"
						, Environment.getExternalStorageDirectory().getPath()+"/certificate");
				try {
					// 인증서 날짜기록 암호화
					enc.HybridEncrypt();
    ...
    private BroadcastReceiver mModeChangeReceiver = new BroadcastReceiver(){
		...
		public void onReceive(Context context, Intent intent) {
			// TODO Auto-generated method stub
			Log.d(TAG,"into ModeChangeReceiver!");
			String action = intent.getAction();
			
			if(action.equals("org.secmem.intent.fake.MODECHANGE_SUCCESS")){
				// 인증 결과 성공으로 락스크린 해제 해주어야 함.
				Log.d("Secmem", "finally first lock screen off!!");
				int timeNum = intent.getIntExtra("time", 0) * 1000;
				Intent myIntent = new Intent("org.secmem.intent.ALARM");
				PendingIntent pi = PendingIntent.getBroadcast(mContext, 0, myIntent, 0);
				AlarmManager mAlarmManager2 = (AlarmManager) mContext.getSystemService(Context.ALARM_SERVICE);
				mAlarmManager2.set(AlarmManager.RTC_WAKEUP, java.lang.System.currentTimeMillis()+timeNum, pi);
				Log.d("Secmem", "Time Tick Event setting finished!!");
				setSecureMode(3);
				synchronized(KeyguardViewMediator.this){
					resetStateLocked();
				}
			} else if (action.equals("org.secmem.intent.fake.MODECHANGE_FAILED")) {
				// 인증 결과 실패로 TextView 리셋 해주어야 함.
				Log.d("Secmem", "finally first lock screen off!!");
				setSecureMode(2);
				synchronized(KeyguardViewMediator.this){
					resetStateLocked();
				}
			} else if (action.equals("org.secmem.intent.fake.SOCKET_FAILED")) {
				// tcpip socket connection fail
				setSecureMode(2);
				synchronized(KeyguardViewMediator.this){
					resetStateLocked();
				}
			} else if (action.equals("org.secmem.intent.ALARM")) {
				// 인증 사용 시간 만료로 락스크린 재가동 시켜주어야 함. 이때 모드값도 lock으로 재설정.
				Log.d("Secmem", "Time Tick Event!!");
				setSecureMode(0);
				UsableModeManager om = (UsableModeManager)context.getSystemService("usable_mode");
				om.setUsableMode(0);
				om.setSocketMode(0);
				if (om.getUsableMode() == 0) {
					synchronized(KeyguardViewMediator.this){
						resetStateLocked();
					}
					showLocked();
    ...
    private BroadcastReceiver mUsbReceiver = new BroadcastReceiver()	{
		...
		public void onReceive(Context context, Intent intent) {
			...
			if(UsbManager.ACTION_USB_ACCESSORY_ATTACHED.equals(action)){
				//usb atteched
				Log.d(TAG,"this is usb attached");
				synchronized(KeyguardViewMediator.this){
					setSecureMode(0);
					doKeyguardLocked();
				}
				...
				//usb detached
				Log.d(TAG,"this is usb detached");
			}
		}
